...
{
  struct file *new = &(current[read_tp()]->pfiles->opened_files[new_fd]), *old = &(current[read_tp()]->pfiles->opened_files[old_fd]);
  if (old_fd < 0 || new_fd < 0)
  {
    sprint("do_dup2: invalid input!");
    return -1;
  }
  if (new_fd == old_fd)
  {
  ...
  if (current[read_tp()]->pfiles->opened_files[new_fd].status == FD_OPENED)
  {
    do_close(new_fd);
  }
  memcpy(new, old, sizeof(struct file));
  return new_fd;
}
...
{
  struct dentry *current_cwd = current[read_tp()]->pfiles->cwd;
...
//  argv_va = argv_va - argv_va % 8; // align 
...
//  current->trapframe->regs.sp = argvs_va - argvs_va % 16; // align
