...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval)
{
  ...
  case CAUSE_STORE_PAGE_FAULT:
    ...
    pagetable_t page_dir = current[read_tp()]->pagetable;
    uint64 va = ROUNDDOWN(stval, PGSIZE);
    pte_t *pte = page_walk(page_dir, va, 1);
    if ((pte != 0) && (*pte & PTE_V) && (*pte & PTE_C))
    {
      void *pa = alloc_page();
      memcpy(pa, (void *)lookup_pa(page_dir, va), PGSIZE);
      *pte = PTE_V | PA2PTE(pa) | prot_to_type(PROT_READ | PROT_WRITE, 1);
      *pte &= ~PTE_C;
    }
    else
    {
      uint64 sp = current[read_tp()]->trapframe->regs.sp;
      if (stval - sp < 32)
        map_pages(page_dir, va, PGSIZE, (uint64)alloc_page(), prot_to_type(PROT_READ | PROT_WRITE, 1));
      else
      {
        sprint("this address: %lx is not available!\n", va);
        sys_user_exit(1);
      }
    }
    break;
...
void rrsched()
{
  ...
  current[read_tp()]->tick_count++;
  if (current[read_tp()]->tick_count >= TIME_SLICE_LEN)
  {
    current[read_tp()]->status = READY;
    current[read_tp()]->tick_count = 0;
