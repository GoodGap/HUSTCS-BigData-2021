...
int map_pages(pagetable_t page_dir, uint64 va, uint64 size, uint64 pa, int perm)
{
       ...
       first <= last; first += PGSIZE, pa += PGSIZE)
  {
    ...
    if (*pte & PTE_V)
    {
      panic("map_pages fails on mapping va (0x%lx) to pa (0x%lx) pte: %lx\n", first, pa, (uint64)*pte);
    }
...
uint64 user_vm_malloc(pagetable_t pagetable, uint64 pre_size, uint64 new_size)
{
  char *memory;
  if(pre_size > new_size)
    return pre_size;
  pre_size = PGROUNDUP(pre_size);
  for(uint64 old = pre_size; old < new_size; old += PGSIZE)
  {
    memory = (char *)alloc_page();
    if(memory == 0)
      panic("failed to user_vm_malloc .\n");
    memset(memory ,0 ,sizeof(uint8) * PGSIZE);
    map_pages(pagetable, pre_size, PGSIZE, (uint64)memory, prot_to_type(PROT_READ | PROT_WRITE,1) );
  }
  return new_size;
}
...
uint64 malloc(int n)
{
    if(init_flag[read_tp()] == 0)
    {
      current[read_tp()]->heap_sz = USER_FREE_ADDRESS_START;
      uint64 addr = current[read_tp()]->heap_sz;
      growprocess(sizeof(MCB));
      pte_t *pte = page_walk(current[read_tp()]->pagetable, addr, 0);
      MCB *first_control_block = (MCB*) PTE2PA(*pte);
      current[read_tp()]->heap_memory_start = (uint64) first_control_block;
      first_control_block->next = first_control_block;
      first_control_block->size = 0;
      current[read_tp()]->heap_memory_last = (uint64)first_control_block;
      init_flag[read_tp()] = 1;
    }
    MCB *head = (MCB *)current[read_tp()]->heap_memory_start;
    MCB *last = (MCB *)current[read_tp()]->heap_memory_last;
    ...
    while (1)
    {
        ...
        {
            head->available = 0;
            return head->offset + sizeof(MCB);
        }
        if(head->next == last) break;
        head = head->next;
    }
   uint64 alloacte_addr = current[read_tp()]->heap_sz;
   growprocess((uint64) (sizeof(MCB) + n + 8));
   pte_t *pte = page_walk(current[read_tp()]->pagetable, alloacte_addr, 0);
   MCB *now = (MCB *)(PTE2PA(*pte) + (alloacte_addr & 0xfff));
   uint64 amo = (8 - ((uint64)now % 8))%8;
   now = (MCB *)((uint64)now + amo);
   ...
   now->next = head->next;
   head->next = now;
   head = (MCB *)current[read_tp()]->heap_memory_start;
   return alloacte_addr + sizeof(MCB);
}

void free(void *firstaddr)
{
    firstaddr = (void *)((uint64)firstaddr - sizeof(MCB));
    pte_t *pte = page_walk(current[read_tp()]->pagetable, (uint64)(firstaddr), 0);
    MCB *now = (MCB *)(PTE2PA(*pte) + ((uint64)firstaddr & 0xfff));
    uint64 amo = (8 - ((uint64)now % 8))%8;
   now = (MCB *)((uint64)now + amo);
    if(now->available == 1)
